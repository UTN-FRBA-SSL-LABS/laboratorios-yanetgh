...
> R: No, el programa muestra $2050 en vez de $2800.
...
> R: carrito_init pone en 0 el contador de productos (cantidad). Es importante  llamarla antes de usar el carrito porque, si no, ese campo quedaría con un valor indefinido (basura de memoria), haciendo que las demás funciones (agregar, contar, total) se comporten de forma impredecible.
...
> R: El test pasa (350 == 350). Esto es porque con cantidad = 1, el bug de carrito_total (que suma solo el precio sin multiplicar por la cantidad) no se nota, ya que precio * 1 = precio.
...
> R: El test falla. Esperaba 700 (350 x 2), pero obtuvo 350. Esto confirma que carrito_total no está multiplicando el precio por la cantidad del producto.
...
> R: En la línea 24: "total += c->items[i].precio;" dentro de carrito_total. Suma solo el precio unitario, sin considerar cuántas unidades hay.


**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería multiplicar el precio por la cantidad de ese producto: total += c->items[i].precio * c->items[i].cantidad;
...
> R: El test falla. El 5to carrito_agregar devuelve 1 (éxito) en vez de 0, permitiendo que se agregue un producto de más al carrito lleno. Esto además provoca un "stack smashing detected" porque se escribe fuera de los límites del array items (buffer overflow).
...
> R: El operador incorrecto es <= (menor o igual). Debería ser < (estrictamente menor), ya que con MAX_ITEMS=4 los índices válidos del array van de 0 a 3.
...
> R: Sí, las líneas 29 y 30, correspondientes a la función carrito_descuento. No se ejecutaron porque ningún test unitario (tests/test_unitarios.c) llama a esa función — solo se usa en el test de integración (test_compra_con_descuento en tests/test_integracion.c), y la cobertura se midió únicamente sobre los tests unitarios.
...
> R: Un test unitario prueba una sola función de forma aislada; uno de integración prueba varias funciones trabajando juntas. El test unitario (test_total_con_cantidad) detectó primero el bug de carrito_total, porque se escribió antes que el de integración, cuando el bug ya estaba corregido.
...
> R: En C, los arrays no verifican índices ni guardan su tamaño en tiempo de ejecución: escribir en items[4] (fuera de un array de 4) simplemente pisa otra memoria, causando el error de "stack smashing" que apareció al correr el test. Python y Java, en cambio, sí chequean los límites en cada acceso y lanzan una excepción controlada en vez de corromper memoria.
...
> R: Escribir tests obliga a pensar en casos concretos (cantidad = 2, carrito lleno) y verifica el comportamiento real del código al ejecutarlo, no lo que parece que hace a simple vista. Mirando el código, el bug de carrito_total pasa fácil desapercibido; con un test que compara el resultado esperado contra el obtenido, el error queda expuesto de forma clara e inmediata.
...
> R: Con cantidad = 1, el bug no se notaba porque sumar solo el precio (sin multiplicar) daba el mismo resultado que precio × 1. El bug recién 
> se hizo visible con cantidad = 2, donde precio × 2 (700) es distinto de solo el precio (350).
